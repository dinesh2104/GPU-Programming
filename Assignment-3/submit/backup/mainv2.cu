...
__global__ void find_minedge(Edge *edges, int V, int E, Edge *mstedge, int start_edge, int *find)
{
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    id = start_edge * blockDim.x + id; // Start from the edge assigned to this thread
    if (id < V)
    {
        for (int i = 0; i < E; i++)
        {
...
__global__ void find_union(int *find, Edge *mstedge, long int *d_ans, int V)
{
...
__global__ void check(Edge *mstedge, int *find, int V, int *flag, int start_edge)
{
    int id = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void dkernel(Edge *edges, int V, int E, Edge *mstedge, int *find, long int *d_ans)
{
    ...
    if (id < V)
    {
        ...
        while (true)
        {
            ...
            // Marking the union in sequential order
            if (threadIdx.x == 0)
            {
                for (int i = 0; i < V; i++)
                {
...
int main()
{
    ...
    while (true)
    {
        ...
        {
            find_minedge<<<cnt_sm, 1024>>>(d_edges, V, E, d_mstedge, i, d_find);
            i += cnt_sm;
        }
        cudaDeviceSynchronize();
        gpuErrchk(cudaPeekAtLastError());

        find_union<<<1, 1>>>(d_find, d_mstedge, d_ans, V);
        cudaDeviceSynchronize();
        gpuErrchk(cudaPeekAtLastError());
        ...
        *flag = 0;
        for (int i = 0; i < block; i++)
        {
            check<<<cnt_sm, 1024>>>(d_mstedge, d_find, V, flag, i);
            i += cnt_sm;
        }
        cudaDeviceSynchronize();
        gpuErrchk(cudaPeekAtLastError());
