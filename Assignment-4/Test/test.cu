...
void printPath(int dest, const std::vector<int> &parent, int source)
{
    ...
    for (int s = 0; s < V; s++)
    {
        ...
        if (s < 10)
        {
            ...
            for (const auto &pair : nodesByDistance)
            {
                ...
                int nextNode = findNextNode(node, h_parent, s);
                printf("To node %d (distance %d): Next node to visit = %d | ",
