...
__device__ volatile unsigned long long d_time = 0; // Device variable to store elapsed time
...
__global__ void compute(int *road, int *wlist_size, long long *wlist, long long *shelter_capacity, population *pop_t, int *next, int *dist, int num_cities, int *flag, int num_roads, long long *drop_count, long long *drop_value, long long *path_size, long long *paths, unsigned long long clock_f)
{

    while (((clock64() - d_time) / clock_f) > 600)
    {
        return;
    }
    ...
    if (tid < *wlist_size)
    {
            ...
            while (oldval != 0)
            {
                ...
                if (oldval == 0)
                {
                    ...
                    if (ans_pop_elderly != 0 || ans_pop_primeage != 0)
                    {
                        ...
                        drop_value[tid * (30 * 3) + drop_count[tid] * 3 + 1] = ans_pop_primeage;
                        drop_value[tid * (30 * 3) + drop_count[tid] * 3 + 2] = ans_pop_elderly;
                        drop_count[tid]++;
                    }
                    lock[city] = 0;
                }
        ...
        for (int i = 0; i < num_cities; i++)
        {
            if (shelter_capacity[i] > 0)
            {
                ...
                // Avoid division by zero or selecting same city
                if (distance > 0)
                {
                    double score = (double)shelter_capacity[i] / (double)distance;
        ...
        if (!is_blocked)
        {
            if (is_within_distance)
            {
                ...
                // Checking whether we can move or not based on simulation.
                unsigned long long time_travel = (round * (next_node_length / 5 * 60)) / (1000000);
                unsigned long long time_diff = (clock64() - pop_t[tid].previous_time) / clock_f;
                // printf("Time diff %lld and time_travel %lld\n", time_diff, time_travel);
                while (((clock64() - pop_t[tid].previous_time) / clock_f) < time_travel)
                    ;

                pop_t[tid].previous_time = clock64();
            ...
            else
            {
                ...
                unsigned long long time_travel = (round * (next_node_length / 5 * 60)) / (1000000);
                unsigned long long time_diff = (clock64() - pop_t[tid].previous_time) / clock_f;
                // printf("2 Time diff %lld and time_travel %lld\n", time_diff, time_travel);
                while (((clock64() - pop_t[tid].previous_time) / clock_f) < time_travel)
                    ;
...
__global__ void compute_v1(int *road, int *wlist_size, long long *wlist, long long *shelter_capacity, population *pop_t, int *next, int *dist, int num_cities, int *flag, int num_roads, long long *drop_count, long long *drop_value, long long *path_size, long long *paths, unsigned long long clock_f)
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    while (tid < *wlist_size && wlist[tid] != -1)
    {
        while (((clock64() - d_time) / clock_f) > 600)
        {
            return;
...
__global__ void timing(unsigned long long clock_f)
{
    d_time = clock64(); // Get the current clock value
    printf("Timing kernel started\n");
}
...
int main(int argc, char *argv[])
{
    ...
    unsigned long long clock_f = static_cast<unsigned long long>(prop.clockRate) * 1000; // Convert from kHz to Hz

    printf("GPU clock frequency: %llu Hz\n", clock_f);
    timing<<<1, 1>>>(clock_f);

    // compute_v1<<<numBlocks1, blockSize1>>>(d_road, d_wlist_size, d_wlist, d_shelter_capacity, d_pop, d_next, d_dist, num_cities, d_flag, num_roads, d_drop, d_drop_value, d_path_size, d_paths, clock_f);
    // cudaDeviceSynchronize();

    while (true)
    {
        cudaMemset(d_flag, 0, sizeof(int));
        compute<<<numBlocks, blockSize>>>(d_road, d_wlist_size, d_wlist, d_shelter_capacity, d_pop, d_next, d_dist, num_cities, d_flag, num_roads, d_drop, d_drop_value, d_path_size, d_paths, clock_f);
