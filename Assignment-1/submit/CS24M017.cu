...
__global__ void CalculateInvertedGrayScale(long int *d_red, long int *d_green, long int *d_blue, long int *d_T1, int m, int n)
{
    ...
    if (i < m && j < n)
    {
        d_T1[((m - i - 1) * n) + j] = floorf((d_red[(i * n) + j] + d_green[(i * n) + j] + d_blue[(i * n) + j]) / 3);
    }
}
...
__global__ void CalculateThomasTransformation(long int *d_red, long int *d_green, long int *d_blue, long int *d_T2, int m, int n)
{
    ...
    if (i < m && j < n)
    {
        long int a = floorf(0.5 * d_red[(i * n) + j]);
        long int b = floorf(sqrtf(d_green[(i * n) + j]));
        d_T2[(i * n) + j] = a + b + d_blue[(i * n) + j];
    }
}
...
int main(int argc, char **argv)
{
    ...
    // printing the T1 matrices and T2 matrices
    // for (int i = 0; i < m; i++)
    // {
    //     for (int j = 0; j < n; j++)
    //     {
